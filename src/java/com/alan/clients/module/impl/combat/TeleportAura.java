...
public final class TeleportAura extends Module {
    ...
    private void doAttack(final List<Entity> targets) {
        if (clickStopWatch.finished(this.nextSwing) && target != null && !InstanceAccess.mc.gameSettings.keyBindAttack.isKeyDown() && !InstanceAccess.mc.gameSettings.keyBindUseItem.isKeyDown()) {
            ...
            switch (this.mode.getValue().getName()) {
                case "Single": {
                    if (InstanceAccess.mc.thePlayer.getDistanceToEntity(target) <= range) {
                        this.attack(target);
                    }
                    break;
                }
    ...
    private void attack(Entity target) {
        ...
        path = MainPathFinder.computePath(new Vec3(InstanceAccess.mc.thePlayer.posX, InstanceAccess.mc.thePlayer.posY, InstanceAccess.mc.thePlayer.posZ), new Vec3(target.posX, target.posY, target.posZ), true);
        ...
        if (this.getModule(Criticals.class).isEnabled() || InstanceAccess.mc.thePlayer.fallDistance > 0 && !InstanceAccess.mc.thePlayer.onGround && !InstanceAccess.mc.thePlayer.isOnLadder() && !InstanceAccess.mc.thePlayer.isInWater() && !InstanceAccess.mc.thePlayer.isPotionActive(Potion.blindness) && InstanceAccess.mc.thePlayer.ridingEntity == null) {
