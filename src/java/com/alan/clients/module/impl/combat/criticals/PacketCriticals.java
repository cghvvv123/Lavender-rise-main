...
public final class PacketCriticals extends Mode<Criticals> {
    ...
    public final Listener<AttackEvent> onAttack = event -> {
        if(stopwatch.finished(delay.getValue().longValue()) && InstanceAccess.mc.thePlayer.onGroundTicks > 2) {
            for (final double offset : offsets) {
                PacketUtil.send(new C03PacketPlayer.C04PacketPlayerPosition(InstanceAccess.mc.thePlayer.posX, InstanceAccess.mc.thePlayer.posY + offset, InstanceAccess.mc.thePlayer.posZ, false));
            }
