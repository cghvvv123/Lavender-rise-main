...
public final class WorldGuardRegen extends Mode<Regen> {
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        if (InstanceAccess.mc.thePlayer.onGround && InstanceAccess.mc.thePlayer.getHealth() < this.health.getValue().floatValue()) {
            if (ticks <= 1) {
    ...
    public final Listener<PacketSendEvent> onPacketSend = event -> {
        ...
        if (p instanceof C02PacketUseEntity && InstanceAccess.mc.thePlayer.onGround && InstanceAccess.mc.thePlayer.getHealth() < this.health.getValue().floatValue() && ticks <= 1) {
    ...
    public final Listener<StrafeEvent> onStrafe = event -> {
        if (InstanceAccess.mc.thePlayer.onGround && InstanceAccess.mc.thePlayer.getHealth() < this.health.getValue().floatValue() && ticks <= 1) {
            event.setSpeed(0);
        }
    };
