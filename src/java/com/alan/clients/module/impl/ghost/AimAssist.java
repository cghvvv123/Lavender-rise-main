...
public class AimAssist extends Module {
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        if (InstanceAccess.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) return;
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        Vector2f rotations = new Vector2f(this.lastRotations.x + (this.rotations.x - this.lastRotations.x) * InstanceAccess.mc.timer.renderPartialTicks, 0);
        final float strength = (float) MathUtil.getRandom(this.strength.getValue().floatValue(), this.strength.getSecondValue().floatValue());

        final float f = InstanceAccess.mc.gameSettings.mouseSensitivity * 0.6F + 0.2F;
        final float gcd = f * f * f * 8.0F;

        int i = InstanceAccess.mc.gameSettings.invertMouse ? -1 : 1;
        float f2 = this.mc.mouseHelper.deltaX + ((rotations.x - InstanceAccess.mc.thePlayer.rotationYaw) * (strength / 100) -
