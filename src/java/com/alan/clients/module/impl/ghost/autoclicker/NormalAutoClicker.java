...
public class NormalAutoClicker extends Mode<AutoClicker> {
    ...
    public final Listener<TickEvent> onTick = event -> {
        ...
        if (clickStopWatch.finished(this.nextSwing) && (!hitSelect.getValue() || ((hitSelect.getValue() && attackTicks >= 10) ||
                (InstanceAccess.mc.thePlayer.hurtTime > 0 && clickStopWatch.finished(this.nextSwing)))) && InstanceAccess.mc.currentScreen == null) {
            final long clicks = (long) (Math.round(MathUtil.getRandom(this.cps.getValue().intValue(), this.cps.getSecondValue().intValue())) * 1.5);

            if (InstanceAccess.mc.gameSettings.keyBindAttack.isKeyDown()) {
                ticksDown++;
            } else {
                ticksDown = 0;
            }
            ...
            if (rightClick.getValue() && InstanceAccess.mc.gameSettings.keyBindUseItem.isKeyDown() && !InstanceAccess.mc.gameSettings.keyBindAttack.isKeyDown()) {
            ...
            if (leftClick.getValue() && ticksDown > 1 && (Math.sin(nextSwing) + 1 > Math.random() || Math.random() > 0.25 || clickStopWatch.finished(4 * 50)) && !InstanceAccess.mc.gameSettings.keyBindUseItem.isKeyDown() && (InstanceAccess.mc.objectMouseOver == null || InstanceAccess.mc.objectMouseOver.typeOfHit != MovingObjectPosition.MovingObjectType.BLOCK)) {
