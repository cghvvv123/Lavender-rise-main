...
public final class FuncraftFlight extends Mode<Flight> {
    ...
    public void onEnable() {
        moveSpeed = 0;
        stage = InstanceAccess.mc.thePlayer.onGround ? 0 : -1;
        ticks = 0;
    }
    ...
    private final Listener<MoveEvent> moveEventListener = event -> {
        if(!MoveUtil.isMoving() || InstanceAccess.mc.thePlayer.isCollidedHorizontally) {
            stage = -1;
        }
        ...
        if(vanillaKickBypass.getValue() && ticks > 125) {
            stage = -1;
            ticks = 0;
            PacketUtil.sendNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(InstanceAccess.mc.thePlayer.posX + 5, InstanceAccess.mc.thePlayer.posY + 1, InstanceAccess.mc.thePlayer.posZ + 5, true));
            return;
        }
        ...
        switch(stage) {
            ...
            case 1:
                if(InstanceAccess.mc.thePlayer.onGround) {
                    event.setPosY(InstanceAccess.mc.thePlayer.motionY = 0.3999);
                    moveSpeed *= 2.14;
                }
                break;
            case 2:
        ...
        InstanceAccess.mc.thePlayer.jumpMovementFactor = 0F;
