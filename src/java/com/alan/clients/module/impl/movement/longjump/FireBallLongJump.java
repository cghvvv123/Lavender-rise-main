...
public class FireBallLongJump extends Mode<LongJump> {
    ...
    public final Listener<PreUpdateEvent> onPreUpdate = event -> {
        ...
        switch (mode.getValue().getName()) {
            case "Hypixel":
                if (InstanceAccess.mc.thePlayer.hurtTime == 10) {
                    yawAtDamage = InstanceAccess.mc.thePlayer.rotationYaw;
                    InstanceAccess.mc.thePlayer.motionY = 1.5;
                    moveSpeed = 1.4;
                }

                if (InstanceAccess.mc.thePlayer.hurtTime == 9) {
                    moveSpeed = 2 - Math.random() / 100f;
                }

                if (InstanceAccess.mc.thePlayer.ticksSinceVelocity <= 11) {
        ...
        if (InstanceAccess.mc.thePlayer.onGroundTicks == 1) {
            MoveUtil.stop();
        }
        ...
        RotationComponent.setRotations(new Vector2f(InstanceAccess.mc.thePlayer.rotationYaw, 90), 10, MovementFix.OFF);
    };
