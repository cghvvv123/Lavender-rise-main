...
public class BlocksMCSpeed extends Mode<Speed> {
    ...
    public final Listener<StrafeEvent> onStrafe = event -> {
        ...
        final boolean potionActive = InstanceAccess.mc.thePlayer.isPotionActive(Potion.moveSpeed);

        if (MoveUtil.isMoving()) {
            switch (InstanceAccess.mc.thePlayer.offGroundTicks) {
                case 0:
                    InstanceAccess.mc.thePlayer.motionY = MoveUtil.jumpBoostMotion(0.42f);
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        PacketUtil.sendNoEvent(new C0BPacketEntityAction(InstanceAccess.mc.thePlayer, C0BPacketEntityAction.Action.START_SPRINTING));
    };
