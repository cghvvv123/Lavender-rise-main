...
public class LegitSpeed extends Mode<Speed> {
    ...
    public final Listener<PreUpdateEvent> preUpdate = event -> {
        switch (rotationExploit.getValue().getName()) {
            case "Rotate (Fully Legit)":
                if (!InstanceAccess.mc.thePlayer.onGround)
                    RotationComponent.setRotations(new Vector2f(InstanceAccess.mc.thePlayer.rotationYaw + 45, InstanceAccess.mc.thePlayer.rotationPitch), 10, MovementFix.NORMAL);
                break;
        ...
        if (noJumpDelay.getValue()) {
            InstanceAccess.mc.thePlayer.jumpTicks = 0;
        }

        if (cpuSpeedUpExploit.getValue()) {
            InstanceAccess.mc.timer.timerSpeed = 1.004f;
        }
    };
