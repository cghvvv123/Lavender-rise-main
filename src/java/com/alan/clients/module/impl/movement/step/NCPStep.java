...
public class NCPStep extends Mode<Step> {
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        if (InstanceAccess.mc.thePlayer.onGround && !PlayerUtil.inLiquid()) {
        ...
        for (int i = 1; i < this.height.getValue().intValue() + 1; i++) {
            InstanceAccess.mc.thePlayer.motionY -= i;
        }
    };
    ...
    public final Listener<StepEvent> onStep = event -> {
        ...
        InstanceAccess.mc.timer.timerSpeed = this.timer.getValue().floatValue();

        for (final double d : values) {
            PacketUtil.send(new C03PacketPlayer.C04PacketPlayerPosition(InstanceAccess.mc.thePlayer.posX, InstanceAccess.mc.thePlayer.posY + (d + (Math.random() / 2000)), InstanceAccess.mc.thePlayer.posZ, false));
        }
