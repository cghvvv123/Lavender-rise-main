...
public final class WatchdogTeleport extends Mode<Teleport> {
    ...
    public final Listener<Render2DEvent> render2D = event -> {
        ...
        FontManager.getProductSansLight(18).
                drawCenteredString(blocks + " blocks required to get to this position, use sneak to activate",
                        InstanceAccess.mc.scaledResolution.getScaledWidth() / 2f, InstanceAccess.mc.scaledResolution.getScaledHeight() / 2f + 30,
    ...
    public final Listener<Render3DEvent> render3D = event -> {
        final float partialTicks = InstanceAccess.mc.timer.renderPartialTicks;

        EntityPlayer player = InstanceAccess.mc.thePlayer;
        ...
        final double yaw = Math.toRadians(InstanceAccess.mc.thePlayer.prevRotationYaw + (InstanceAccess.mc.thePlayer.rotationYaw - InstanceAccess.mc.thePlayer.prevRotationYaw) * partialTicks);
    ...
    public final Listener<PreUpdateEvent> preUpdate = event -> {
        blocks = (int) (position.yCoord - (InstanceAccess.mc.thePlayer.posY - 2));

        if (!InstanceAccess.mc.gameSettings.keyBindSneak.isKeyDown()) return;
