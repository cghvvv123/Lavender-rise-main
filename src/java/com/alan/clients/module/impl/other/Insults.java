...
public final class Insults extends Module {
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        if (target != null && !InstanceAccess.mc.theWorld.playerEntities.contains(target)) {
            ...
            if (ticks >= delay.getValue().intValue() + Math.random() * 5 && !BadPacketsComponent.bad()) {
                ...
                InstanceAccess.mc.thePlayer.sendChatMessage(String.format(insult, PlayerUtil.name(target)));
                target = null;
            }
