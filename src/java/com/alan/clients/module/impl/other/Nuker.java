...
public final class Nuker extends Module {
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        switch (mode.getValue().getName()) {
            ...
            case "Teleport":
                if (InstanceAccess.mc.gameSettings.keyBindUseItem.isKeyDown() && stopWatch.finished(delay.getValue().longValue())) {
                    final BlockPos target = InstanceAccess.mc.thePlayer.rayTrace(999, 1).getBlockPos();

                    if (InstanceAccess.mc.theWorld.getBlockState(target).getBlock() instanceof BlockAir) {
                        return;
                    }

                    if (InstanceAccess.mc.thePlayer.capabilities.allowFlying && !InstanceAccess.mc.thePlayer.capabilities.isFlying) {
                        final PlayerCapabilities capabilities = InstanceAccess.mc.thePlayer.capabilities;
    ...
    @EventLink()
    public final Listener<WorldChangeEvent> onWorldChange = event -> this.toggle();
        ...
        for (double posX = -range; posX < range; posX++) {
            for (double posY = range; posY > -range; posY--) {
                for (double posZ = -range; posZ < range; posZ++) {
                    if (scatter.getValue() && !((InstanceAccess.mc.thePlayer.ticksExisted % 2 == 0 ? posX : posZ) % 2 == 0)) {
                        continue;
                    }

                    final BlockPos blockPos = new BlockPos(x + posX, y + posY, z + posZ);
                    final Block block = InstanceAccess.mc.theWorld.getBlockState(blockPos).getBlock();
