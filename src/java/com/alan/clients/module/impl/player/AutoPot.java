...
public class AutoPot extends Module {
    ...
    public final Listener<PreUpdateEvent> onPreUpdate = event -> {
        ...
        if (InstanceAccess.mc.thePlayer.onGroundTicks <= 1 || attackTicks < 10 || this.getModule(Scaffold.class).isEnabled()) {
            return;
        }
            ...
            if (item instanceof ItemPotion) {
                ...
                if (!ItemPotion.isSplash(stack.getMetadata()) ||
                        ...
                        (effect.getPotionID() == Potion.regeneration.id ||
                                ...
                                InstanceAccess.mc.thePlayer.getHealth() > this.health.getValue().floatValue()) {
                    continue;
                }

                if (InstanceAccess.mc.thePlayer.isPotionActive(effect.potionID) &&
                        InstanceAccess.mc.thePlayer.getActivePotionEffect(effect.potionID).duration != 0) {
                    continue;
                }
                ...
                RotationComponent.setRotations(new Vector2f(InstanceAccess.mc.thePlayer.rotationYaw, 90), rotationSpeed, MovementFix.NORMAL);
