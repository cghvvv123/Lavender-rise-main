...
public class BlinkAntiVoid extends Mode<AntiVoid> {
    ...
    public final Listener<PreUpdateEvent> onPreUpdate = event -> {
        ...
        if (InstanceAccess.mc.thePlayer.ticksExisted <= 50) return;
        ...
        boolean overVoid = !InstanceAccess.mc.thePlayer.onGround && !PlayerUtil.isBlockUnder(30, true);

        if (overVoid) {
            overVoidTicks++;
        } else if (InstanceAccess.mc.thePlayer.onGround) {
            overVoidTicks = 0;
        }
        ...
        if (overVoid && position != null && motion != null && overVoidTicks < 30 + distance.getValue().doubleValue() * 20) {
            ...
            motion = new Vec3(InstanceAccess.mc.thePlayer.motionX, InstanceAccess.mc.thePlayer.motionY, InstanceAccess.mc.thePlayer.motionZ);
