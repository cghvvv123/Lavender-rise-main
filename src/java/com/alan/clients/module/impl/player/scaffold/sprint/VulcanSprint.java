...
public class VulcanSprint extends Mode<Scaffold> {
    ...
    public void onDisable() {
        if (time == 1) {
            PacketUtil.send(new C0BPacketEntityAction(InstanceAccess.mc.thePlayer, C0BPacketEntityAction.Action.STOP_SNEAKING));
        }
    }
    ...
    public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
        ...
        if (!InstanceAccess.mc.gameSettings.keyBindJump.isKeyDown() && speed > 0.9) {
            if (InstanceAccess.mc.thePlayer.onGroundTicks >= 2 && block <= 10) {
                MoveUtil.strafe(MoveUtil.getAllowedHorizontalDistance() * speed);

                InstanceAccess.mc.thePlayer.jump();
                InstanceAccess.mc.thePlayer.motionY = 0.012500047683714;
            }
        }
        ...
        switch (time) {
            case 1:
                PacketUtil.send(new C0BPacketEntityAction(InstanceAccess.mc.thePlayer, C0BPacketEntityAction.Action.START_SNEAKING));
                break;
