...
public class NormalTower extends Mode<Scaffold> {
    ...
    public final Listener<PacketSendEvent> onPacketSend = event -> {
        ...
        if (InstanceAccess.mc.thePlayer.motionY > -0.0784000015258789 && packet instanceof C08PacketPlayerBlockPlacement) {
            final C08PacketPlayerBlockPlacement wrapper = ((C08PacketPlayerBlockPlacement) packet);

            if (wrapper.getPosition().equals(new BlockPos(InstanceAccess.mc.thePlayer.posX, InstanceAccess.mc.thePlayer.posY - 1.4, InstanceAccess.mc.thePlayer.posZ))) {
                InstanceAccess.mc.thePlayer.motionY = -0.0784000015258789;
            }
        }
    };
}
