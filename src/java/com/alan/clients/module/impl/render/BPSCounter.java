...
public final class BPSCounter extends Module {
    ...
    public final Listener<PostStrafeEvent> onPostStrafe = event -> {
        speed = MathUtil.round((MoveUtil.speed() * 20) * InstanceAccess.mc.timer.timerSpeed, 1) + "";
    };
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        scale.x = titleWidth + InstanceAccess.nunitoNormal.width(bpsString);
        ...
        InstanceAccess.NORMAL_POST_RENDER_RUNNABLES.add(() -> {
            ...
            final double textY = position.y + scale.y / 2.5f - InstanceAccess.nunitoNormal.height() / 8.0F;
            InstanceAccess.nunitoNormal.drawStringWithShadow(titleString, textX, textY, getTheme().getFirstColor().getRGB());
            InstanceAccess.nunitoNormal.drawStringWithShadow(bpsString, textX + titleWidth, textY, Color.WHITE.getRGB());
        });

        InstanceAccess.NORMAL_BLUR_RUNNABLES.add(() -> RenderUtil.roundedRectangle(position.x, position.y, scale.x + 6, scale.y - 1, getTheme().getRound(), Color.BLACK));
        InstanceAccess.NORMAL_POST_BLOOM_RUNNABLES.add(() -> RenderUtil.roundedRectangle(position.x, position.y, scale.x + 6, scale.y - 1, getTheme().getRound() + 1, getTheme().getDropShadow()));
    };
}
