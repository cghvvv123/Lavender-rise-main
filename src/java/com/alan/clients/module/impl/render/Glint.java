    ...
    public void renderEffect(final IBakedModel model) {
        ...
        if (!Config.isCustomItems() || CustomItems.isUseGlint()) {
            if (!Config.isShaders() || !Shaders.isShadowPass) {
                ...
                InstanceAccess.mc.getRenderItem().textureManager.bindTexture(RenderItem.RES_ITEM_GLINT);

                if (Config.isShaders() && !InstanceAccess.mc.getRenderItem().renderItemGui) {
                ...
                for (int layer = 1; layer <= layers.getValue().intValue(); layer++) {
                    ...
                    InstanceAccess.mc.getRenderItem().renderModel(model, ColorUtil.mixColors(firstColor, secondColor, (layer / layers.getValue().floatValue())).hashCode());
                }
                ...
                InstanceAccess.mc.getRenderItem().textureManager.bindTexture(TextureMap.locationBlocksTexture);
