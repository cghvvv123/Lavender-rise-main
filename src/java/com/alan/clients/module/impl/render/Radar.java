...
public class Radar extends Module {
    ...
    private final ResourceLocation radarBeamResource = new ResourceLocation("lavender/icons/radar/radarbeam.png");
    private final ResourceLocation radarDotResource = new ResourceLocation("lavender/icons/radar/radardot.png");
    private final ResourceLocation radarScopeResource = new ResourceLocation("lavender/icons/radar/radarscope.png");
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        if (InstanceAccess.mc.gameSettings.showDebugInfo) return;
        ...
        InstanceAccess.NORMAL_POST_RENDER_RUNNABLES.add(() -> {
            ...
            GL11.glRotated(-InstanceAccess.mc.thePlayer.rotationYaw + 180, 0, 0, 1);

            // Moved above loop (why calc for every entity xd)
            double playerX = InstanceAccess.mc.thePlayer.prevPosX + (InstanceAccess.mc.thePlayer.posX - InstanceAccess.mc.thePlayer.prevPosX) * event.getPartialTicks();
            double playerZ = InstanceAccess.mc.thePlayer.prevPosZ + (InstanceAccess.mc.thePlayer.posZ - InstanceAccess.mc.thePlayer.prevPosZ) * event.getPartialTicks();

            for (Entity entity : InstanceAccess.mc.theWorld.getLoadedEntityList()) {
                if (entity.getDistanceToEntity(InstanceAccess.mc.thePlayer) > 30) continue; // someone explain why it has to be hardcoded
                if (entity == InstanceAccess.mc.thePlayer) continue;
