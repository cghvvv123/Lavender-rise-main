...
public final class ScoreBoard extends Module {
    private final BooleanValue MC = new BooleanValue("CNFont", this, false);
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        if(rendertitle()){
            com.alan.clients.util.font.FontManager.getProductSansLight(20).drawString("ScoreBoard", position.x+2, position.y-12,new Color(255,255,255,200).getRGB());
        }
        if (this.scoreObjective != null) {
            LIMITED_PRE_RENDER_RUNNABLES.add(() -> this.renderScoreboard(position.x, position.y,  getTheme().getBackgroundShade(), true));
        }
        NORMAL_BLUR_RUNNABLES.add(() -> this.renderScoreboard(position.x, position.y,Color.BLACK, false));
    ...
    private void renderScoreboard(int x, int y, final Color backgroundColor, final boolean font) {
        final RapeMasterFontManager fontRenderer = FontManager.arial18;
        int fontHeight = FontRenderer.FONT_HEIGHT;
        final int size = collection.size();
        final int padding = 3;
        int height = fontHeight * size + padding;
        ...
        if(MC.getValue()) {
            ...
            height = fontHeight * size + padding;
            ...
            final int fontColor = Color.WHITE.getRGB();
            x += padding * 2;
            y += padding + 1.5;

            // draws title of scoreboard
            final String objective = scoreObjective.getDisplayName();
            fontRenderer.drawString(objective, x + maxWidth / 2.0F - fontRenderer.getStringWidth(objective) / 2.0F, y, fontColor);
            ...
            // draws title of scoreboard
            final String objective = scoreObjective.getDisplayName();
            mc.fontRendererObj.drawString(objective, x + maxWidth / 2.0F - mc.fontRendererObj.width(objective) / 2.0F, y, fontColor);
            ...
            for (final Score score1 : collection) {
                ...
                final ScorePlayerTeam scorePlayerTeam = this.scoreObjective.getScoreboard().getPlayersTeam(score1.getPlayerName());
                final String s1 = ScorePlayerTeam.formatPlayerName(scorePlayerTeam, score1.getPlayerName());
                mc.fontRendererObj.drawString(s1, x, y, fontColor);
            }
        }
