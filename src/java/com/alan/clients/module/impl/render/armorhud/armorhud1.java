...
public class armorhud1 extends Mode<ArmorHud> {
    public armorhud1(String name, ArmorHud parent) {
        super(name, parent);
    }
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        anmitY = (float) RenderUtil.getAnimationState(anmitY,((this.modeValue.getValue().getName().equalsIgnoreCase("Vertical") && selfadaption.getValue() ? (int) (scale.y + heightfix) :65)), 90.0);
        ...
        NORMAL_POST_RENDER_RUNNABLES.add(() -> {
            ...
            if(ArmorHud.rendertitle()){
                FontManager.getProductSansLight(20).drawString("ArmorHud", textX-4.5, textY-16,new Color(255,255,255,200).getRGB());
            }
            if (this.modeValue.getValue().getName().equalsIgnoreCase("Vertical")) {
                RenderUtil.roundedRectangle(position.x, position.y, scale.x + 22,anmitY , getTheme().getRound(), getTheme().getBackgroundShade());
            }else {
                RenderUtil.roundedRectangle(position.x, position.y, Width,43 , getTheme().getRound(), getTheme().getBackgroundShade());
            }
            ...
            for (int index = 3; index >= 0; index--) {
                ...
                if (stack != null) {
                    if (this.modeValue.getValue().getName().equalsIgnoreCase("Vertical")) {
                        FontManager.getPoppinsRegular(14).drawStringWithShadow(Integer.toString(stack.getMaxDamage() - stack.getItemDamage()),
                                textX + 18, rendery + 15.5, Color.white.getRGB());
                        renderItem.renderItemIntoGUI(stack, textX - 2, rendery + 10);
                    }else {
                        FontManager.getPoppinsRegular(13).drawStringWithShadow(Integer.toString(stack.getMaxDamage() - stack.getItemDamage()),
                                renderx+1, textY + 29, Color.white.getRGB());
            ...
            if(!isPlayerWearingArmor){
                ...
                if (this.modeValue.getValue().getName().equalsIgnoreCase("Vertical")) {
                    FontManager.getTahoma(16).drawString("Empty..", textX, textY + 16, -1);
                }else{
                    FontManager.getTahoma(16).drawString("Empty..", textX, textY + 19, -1);
                }
                ...
                heightfix = 31;Widthfix =19;
            }else  if(armorCount == 3) {
                heightfix = 48;Widthfix =35;
            }else  if(armorCount == 4) {
                heightfix = 65;Widthfix =53;
            }
        ...
        NORMAL_BLUR_RUNNABLES.add(() -> {
            if (this.modeValue.getValue().getName().equalsIgnoreCase("Vertical")) {
                RenderUtil.roundedRectangle(position.x, position.y, scale.x + 22,anmitY, getTheme().getRound(), Color.BLACK);
            }else {
                RenderUtil.roundedRectangle(position.x, position.y, Width,43 , getTheme().getRound(), Color.BLACK);
            }
        });
        NORMAL_POST_BLOOM_RUNNABLES.add(() -> {
            if (this.modeValue.getValue().getName().equalsIgnoreCase("Vertical")) {
                RenderUtil.roundedRectangle(position.x, position.y, scale.x + 22, anmitY, getTheme().getRound() + 1, getTheme().getDropShadow());
            }else {
                RenderUtil.roundedRectangle(position.x, position.y, Width,43 , getTheme().getRound(),  getTheme().getDropShadow());
            }
        });
    };
