...
public final class EffectFullBright extends Mode<FullBright> {
    ...
    public final Listener<TickEvent> onTick = event -> {
        InstanceAccess.mc.thePlayer.addPotionEffect(new PotionEffect(Potion.nightVision.id, Integer.MAX_VALUE, 1));
    };

    @Override
    public void onDisable() {
        if (InstanceAccess.mc.thePlayer.isPotionActive(Potion.nightVision)) {
            InstanceAccess.mc.thePlayer.removePotionEffect(Potion.nightVision.id);
        }
    }
}
