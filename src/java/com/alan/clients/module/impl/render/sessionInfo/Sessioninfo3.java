...
public class Sessioninfo3 extends Mode<SessionInfo> {
    public Sessioninfo3(String name, SessionInfo parent) {
        super(name, parent);
    }
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        final String titleString =interfaceModule.Render3cn.getValue() && showTitle.getValue() ?
                Localization.get("玩家信息") : showTitle.getValue() ?Localization.get("SessionInfo"):"";
        double padding = 2;
        EntityPlayer player2 = mc.thePlayer;

        NORMAL_POST_RENDER_RUNNABLES.add(() -> {
            float width = scale.x + 147;
            float height = scale.y + 47;
            ...
            if ((double)interfaceModule.radius.getValue() == 0f)
                RenderUtil.rectangle(SessionInfo.position.x, SessionInfo.position.y, width, height, interfaceModule.getRENDER3BG2Color());
            else RenderUtil.roundedRectangle(SessionInfo.position.x, SessionInfo.position.y, width, height,
                    (double)interfaceModule.radius.getValue(),interfaceModule.getRENDER3BG2Color());

            bg(SessionInfo.position.x, SessionInfo.position.y, width, height,15, (double)interfaceModule.radius.getValue(),interfaceModule.getRENDER3BgColor());

            if(interfaceModule.Render3cn.getValue())
                com.alan.clients.fontRender.FontManager.arial17bold.drawString(titleString,
                        (float) (FontManager.geticon(20).width("b")+textX+4),
                        (float) (textY+2), interfaceModule.getRENDER3fontColor().getRGB());else
            FontManager.getNunitoBold(20).drawString(titleString, FontManager.geticon(20).width("b")+textX+4,
                    textY+3.5, interfaceModule.getRENDER3fontColor().getRGB());

            FontManager.geticon(21).drawString("b", textX+1, textY+2.5,interfaceModule.getRENDER3fontColor().getRGB());

            renderTargetHead(player2, (float) textX, (float) (SessionInfo.position.y + 17+padding),45);

            RenderUtil.circle(textX, SessionInfo.position.y + 17+padding, 45, 360, false, new Color(0, 0, 0, 70));
            RenderUtil.circle(textX, SessionInfo.position.y + 17+padding, 45, (System.currentTimeMillis() - this.startTime) * 0.00001,
                    false,  Color.white);

            FontManager.getProductSansBold(18).drawStringWithShadow(username, textX + padding+43,
                    SessionInfo.position.y + padding + 21, new Color(255, 255, 255, 210).getRGB());

            FontManager.getProductSansBold(18).drawStringWithShadow(time, textX + padding+47,
                    SessionInfo.position.y + padding + 37, new Color(255, 255, 255, 180).getRGB());

            FontManager.getProductSansMedium(18).drawStringWithShadow( "kills " + HytHelper.kill, textX + padding+44,
                    SessionInfo.position.y + padding + 52, new Color(255, 255, 255, 180).getRGB());

            FontManager.getProductSansMedium(18).drawStringWithShadow( "wins " + HytAutoPlay.wins, textX + padding + 73,
                    SessionInfo.position.y + padding + 52, new Color(255, 255, 255, 180).getRGB());

        });
        //NORMAL_POST_BLOOM_RUNNABLES.add(() -> RenderUtil.rectangle(SessionInfo.position.x, SessionInfo.position.y, scale.x + 147, scale.y + 46, getTheme().getDropShadow()));
        NORMAL_BLUR_RUNNABLES.add(() -> {
            if((double)interfaceModule.radius.getValue() == 0f){
                RenderUtil.rectangle(SessionInfo.position.x, SessionInfo.position.y, scale.x + 147,
                        scale.y + 47, Color.BLACK);
            }else {
                RenderUtil.roundedRectangle(SessionInfo.position.x, SessionInfo.position.y, scale.x + 147,
                        scale.y + 47, (double)interfaceModule.radius.getValue(),Color.BLACK);
            }
        });
        NORMAL_POST_BLOOM_RUNNABLES.add(() -> {
            ...
            if((double)interfaceModule.radius.getValue() == 0f){
                RenderUtil.rectangle(SessionInfo.position.x, SessionInfo.position.y, scale.x + 147,
                        scale.y + 47, interfaceModule.getbloomBG2Color());
            }else {
                RenderUtil.roundedRectangle(SessionInfo.position.x, SessionInfo.position.y, scale.x + 147,
                        scale.y + 47, (double)interfaceModule.radius.getValue(), interfaceModule.getbloomBG2Color());
            }
            // title in the top left corner of the screen
            //RenderUtil.rectangle(4, 4.5F, Lavendernw4 + t18.width(Lavenderstr4) + 1f, t18.height() +3,  getTheme().getDropShadow());
        });
    };
    ...
    private void bg(double x, double y, double width,  double height,double height2, double radius, Color color) {
        ...
        FontManager.geticn(130).drawString("A", x+(width/2)+32, y+(height/2)-20, new Color(210, 210, 210,40).getRGB());
        StencilUtil.uninitStencilBuffer();
    }
