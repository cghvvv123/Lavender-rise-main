...
//        RenderUtil.dropShadow(3, (float) position.x, (float) position.y, scale.x, scale.y, 42, 1);
        ...
        if (clickGUI.getSelectedScreen() instanceof SearchScreen) {
            FontManager.arial16.drawString("(" + Localization.get(module.getModuleInfo().category().getName()) + ")",
                    (float) (position.getX() + FontManager.arial20.getStringWidth(Localization.get(this.module.getModuleInfo().name())) + 10F),
                    (float) position.getY() + 10, ColorUtil.withAlpha(fontColor, 64).hashCode());
        }

        // Draw module name
        FontManager.arial20.drawString(Localization.get(this.module.getModuleInfo().name()), (float) position.x + 6f, (float) position.y + 8,
                module.isEnabled() ? getTheme().getAccentColor(new Vector2d(0, position.y / 5)).getRGB() : fontColor.getRGB());

        // Draw module category
        FontManager.arial16.drawString(Localization.get(module.getModuleInfo().description()), (float) position.x + 6f,
