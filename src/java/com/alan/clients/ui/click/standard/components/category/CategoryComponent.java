...
import static com.alan.clients.util.animations.Easing.LINEAR;
...
public final class CategoryComponent implements InstanceAccess {
    ...
    public void render(final double offset, final double sidebarWidth, final double opacity, final Screen selectedScreen) {
        ...
        final double width = FontManager.arial16.getStringWidth(Localization.get(category.getName())) + spacer + category.getFontRenderer().width(category.getIcon());
...
//        }
        ...
        FontManager.arial16.drawString(Localization.get(category.getName()), (float) (x + animation.getValue() / 80f + 3 + spacer) +
                com.alan.clients.util.font.FontManager.getIconsOne(17).width(category.getIcon()), y, color);
    ...
    public void bloom(final double opacity) {
        final double width = FontManager.arial16.getStringWidth(Localization.get(category.getName())) + 12;
        RenderUtil.roundedRectangle(x + 1.5, y - 5.5, width + 9, 15, 4,
                ColorUtil.withAlpha(getTheme().getAccentColor(new Vector2d(0, y / 5D)), (int) (Math.min(animation.getValue(), opacity))).darker());
    }
