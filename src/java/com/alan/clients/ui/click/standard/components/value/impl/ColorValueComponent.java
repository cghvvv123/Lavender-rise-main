...
public class ColorValueComponent extends ValueComponent {
    ...
    public void draw(final Vector2d position, final int mouseX, final int mouseY, final float partialTicks) {
        ...
        final float valueWidth = FontManager.arial16.getStringWidth(this.value.getName()) + 4;

        FontManager.arial16.drawString(this.value.getName(), (float) this.position.x, (float) this.position.y, this.getStandardClickGUI().fontDarkColor.hashCode());
...
//                RenderUtil.rectangle(x, y + pickerHeight - 2, pickerWidth - edge * 2, 5, Color.BLACK);
            ...
            FontManager.arial18.drawCenteredString(color.getRed() + "", (float) (textX + padding), (float) textY, getStandardClickGUI().fontDarkColor.hashCode());
            FontManager.arial18.drawCenteredString(color.getGreen() + "", (float) (textX + 30), (float) textY, getStandardClickGUI().fontDarkColor.hashCode());
            FontManager.arial18.drawCenteredString(color.getBlue() + "", (float) (textX + padding * 6), (float) textY, getStandardClickGUI().fontDarkColor.hashCode());

            textY += 13;

            FontManager.arial14.drawString(String.format("#%02X%02X%02X", color.getRed(), color.getGreen(), color.getBlue()), (float) textX, (float) textY, new Color(55, 59, 61).hashCode());
//            });
        }
    }
