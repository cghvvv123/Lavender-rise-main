package com.alan.clients.ui.cloudmusic.ui;


import com.alan.clients.fontRender.FontManager;
import com.alan.clients.ui.cloudmusic.MusicManager;
import com.alan.clients.ui.cloudmusic.utils.CloudMusicAPI;
import com.alan.clients.ui.cloudmusic.utils.Track;
...
public class GuiCloudMusic extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (Math.ceil(sidebarAnimation) > 1) {
            ...
            FontManager.arial13.drawString("导入", newWidth - 23f, y + 8f, new Color(240, 240, 240).getRGB());

            if (textField.textString.isEmpty()) {
                FontManager.arial13.drawString("输入歌单ID", newX + 8, y + 8f, Color.WHITE.getRGB());
            }
            ...
            this.scrollAni = (float) (float) RenderUtil.getAnimationState(this.scrollAni, scrollY, Math.max(10, (Math.abs(this.scrollAni - (scrollY))) * 50) * 0.3f);
        ...
        FontManager.arial16.drawString("网易云音乐", x + 15, y + 8, Color.BLACK.getRGB());
        ...
        RenderUtil.circle2(x + 10 + (130 * volume), y + 26, 3, new Color(255,40,40).getRGB());
        ...
        if (MusicManager.INSTANCE.loadingThread != null) {
            ...
            RenderUtil.circle2(x + 10 + (1.3f * MusicManager.INSTANCE.downloadProgress), y + height - 48, 3, new Color(255, 50, 50, 255).getRGB());
            RenderUtil.circle2(x + 10 + (1.3f * MusicManager.INSTANCE.downloadProgress), y + height - 48, 2, Color.WHITE.getRGB());
        } else {
            RenderUtil.drawRoundedRect(x + 10, y + height - 50, x + 10 + (1.3f * progress), y + height - 46, 1.4f, new Color(255,40,40).getRGB());
            RenderUtil.circle2(x + 10 + (1.3f * progress), y + height - 48, 3, new Color(255,40,40).getRGB());
            RenderUtil.circle2(x + 10 + (1.3f * progress), y + height - 48, 2, new Color(255,255,255).getRGB());
        }

        //按钮
        RenderUtil.circle2(x + (width / 2), y + height - 19.5f, 12, new Color(255,40,40).brighter().getRGB()); //播放和暂停

        FontManager.micon15.drawString("R", x + 5, y + 8.5f, Color.BLACK.getRGB());
        ...
        FontManager.arial16.drawString(songName, x + (width / 2) - (FontManager.arial16.getStringWidth(songName) / 2f) - 1.5f, y + (height / 2 - 90), Color.BLACK.getRGB());
        FontManager.arial13.drawString(songArtist, x + (width / 2) - (FontManager.arial13.getStringWidth(songArtist) / 2f) - 1.5f, y + (height / 2 - 82), Color.BLACK.getRGB());
        GL11.glDisable(GL11.GL_SCISSOR_TEST);

        if (MusicManager.INSTANCE.getMediaPlayer() != null) {

            if (MusicManager.INSTANCE.getMediaPlayer().getStatus() == Status.PLAYING) {
                FontManager.micon30.drawString("K", x + (width / 2) - (FontManager.micon30.getStringWidth("K") / 2f), y + height - 23, Color.WHITE.getRGB());
            } else {
                FontManager.micon30.drawString("J", x + (width / 2) - (FontManager.micon30.getStringWidth("J") / 2f) + 1, y + height - 23, Color.WHITE.getRGB());
            }

        } else {
            FontManager.micon30.drawString("J", x + (width / 2) - (FontManager.micon30.getStringWidth("J") / 2f) + 1, y + height - 23f, Color.WHITE.getRGB());
        }

        FontManager.micon30.drawString("L", x + width / 2 - (FontManager.micon30.getStringWidth("L") / 2f) - 30, y + height - 23, Color.BLACK.getRGB());
        FontManager.micon30.drawString("M", x + width / 2 - (FontManager.micon30.getStringWidth("M") / 2f) + 27.5f, y + height - 23, Color.BLACK.getRGB());

        if (MusicManager.INSTANCE.repeat) {
            FontManager.micon15.drawString("O", x + width - 20, y + height - 26f, Color.BLACK.getRGB());
        } else {
            FontManager.micon15.drawString("N", x + width - 20, y + height - 26f, new Color(50,50,50).getRGB());
        }

        if (MusicManager.INSTANCE.lyric) {
            FontManager.micon15.drawString("P", x + 10, y + height - 26f, Color.BLACK.getRGB());
        } else {
            FontManager.micon15.drawString("P", x + 10, y + height - 26f, new Color(50,50,50).getRGB());
        }

        if (MusicManager.INSTANCE.currentTrack != null) {
            if (MusicManager.INSTANCE.getArt(MusicManager.INSTANCE.currentTrack.id) != null) {
                GL11.glPushMatrix();
                RenderUtil.drawImage2(MusicManager.INSTANCE.getArt(MusicManager.INSTANCE.currentTrack.id), x + (width / 2) - 50, y + (height / 2 - 10) - 50, 100, 100, 1f);
                GL11.glPopMatrix();
            }
        }
