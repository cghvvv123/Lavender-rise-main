...
public final class AltManagerMenu extends Menu {
    ...
    private final StopWatch click = new StopWatch();
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        // Blur and bloom box
        double boxX = (double) this.width / 2 - ACCOUNT_WIDTH - ACCOUNT_SPACING / 2 - BOX_SPACING;
                            ...
                            if (account.getPassword().equalsIgnoreCase("Offline")) {
                                try {
                                    ...
                                    if (uuid == null || uuid.isEmpty()) {
                                        uuid = RandomUtil.randomName();
                                        account.setUuid(uuid);
                                    }
                                    mc.session = new Session(account.getUsername(), uuid, account.getRefreshToken(), "mojang");
    ...
    public void initGui() {
        ...
        this.loginThroughBrowserButton = new MenuFeedBackTextButton(buttonX, buttonY, BUTTON_WIDTH, BUTTON_HEIGHT, () -> {
            ...
            if (clipboardContent.length() <= 50) {
                try {
                    mc.session = new Session(clipboardContent, RandomUtil.randomName(), "", "mojang");
                } catch (IllegalArgumentException e) {
                    e.printStackTrace();
                }
            }
        ...
        this.loginOfflineAltButton = new MenuFeedBackTextButton(buttonX2, buttonY, BUTTON_WIDTH, BUTTON_HEIGHT, () -> {
            final String name = getClipboardString().isEmpty() || getClipboardString().length() > 30 ? RandomUtil.randomName() : getClipboardString();
            Account account = new Account(name, "Offline");
            account.setOfflineUsername(name);
            account.setRefreshToken("0");
            try {
                String uuid = RandomUtil.randomName();
                account.setUuid(uuid);
                mc.session = new Session(account.getUsername(), uuid, account.getRefreshToken(), mc.session.getSessionType().name());
                Client.INSTANCE.getAltManager().getAccounts().add(account);
