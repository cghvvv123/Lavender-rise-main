...
import com.alan.clients.Loader;
...
import com.alan.clients.newevent.impl.input.KeyEvent;
...
//        this.toggleFullscreen();
      ...
      /*  try {
            if (Browser.get("https://gitcode.net/m0_74037382/emperor/-/raw/master/DevList").contains(GuiLogin.HWID)) {
                GuiLogin.TesterList = Client.name;
                GuiLogin.isload = true;
                GuiLogin.rank = "Dev";
                theMinecraft.displayGuiScreen(new MainMenu());
                Display.setTitle(Client.NAME + " " + Client.VERSION);
                try {
                    SystemUtils.displayTray("DEV认证通道成功", "欢迎，DEV！", TrayIcon.MessageType.INFO);
    ...
    private void createDisplay() throws LWJGLException {
        ...
        try {
            Display.create((new PixelFormat()).withDepthBits(24));} catch (final LWJGLException lwjglexception) {
    ...
    public void drawSplashScreen(int much) {
        ...
        RapeMasterFontManager fontRenderer = FontManager.arial40;
        fontRenderer.drawCenteredString(Client.NAME, sr.getScaledWidth() / 2, sr.getScaledHeight() / 2 - 15, -1);
        drawRect(sr.getScaledWidth() / 2 - 50, sr.getScaledHeight() / 2 + 9, sr.getScaledWidth() / 2 + 50, sr.getScaledHeight() / 2 + 10, Color.GRAY.getRGB());
        drawRect(sr.getScaledWidth() / 2 - 50, sr.getScaledHeight() / 2 + 9, sr.getScaledWidth() / 2 - 50 + much, sr.getScaledHeight() / 2 + 10, Color.WHITE.getRGB());
        GlStateManager.disableLighting();
        GlStateManager.disableFog();
        ...
        much = much / 10;
        Display.setTitle("正在加载 " + much + "/10");
        if (much == 10)
            Display.setTitle(Client.NAME + " " + Client.VERSION_FULL + " | LWJGL Version: " + Sys.getVersion());
        this.updateDisplay();
    }
...
//                        }
    ...
    public void runTick() throws IOException {
        ...
        if (!this.isGamePaused) {
            threadPool.execute(() -> this.ingameGUI.updateTick());
        }
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        Client.INSTANCE.getEventBus().handle(new KeyEvent(k));
                        switch (k) {
                            case 1:
