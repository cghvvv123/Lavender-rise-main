...
import com.alan.clients.module.impl.render.FakeBody;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        FakeBody fake = FakeBody.INSTANCE;
        int max = fake.tick.getValue().intValue() * fake.count.getValue().intValue() + 1;

        if(this instanceof RenderPlayer && fake.isEnabled()){
            if(!map.containsKey(entity)){
                map.put(entity,new ArrayList<>());
            }
            ArrayList<RenderData> list = (ArrayList) map.get(entity);
            if(list.size() >= max){
                for(int i = 0 ; i < max-1 ; i += fake.tick.getValue().intValue()){
                    renderFake(entity,list.get(i));
                }
            }
        }
