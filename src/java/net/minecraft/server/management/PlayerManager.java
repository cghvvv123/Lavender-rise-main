    ...
    public void updatePlayerInstances() {
        ...
        if (j - this.previousTotalWorldTime > 8000L) {
            ...
            for (int k = 0; k < this.playerInstanceList.size(); ++k) {
                if (mc.thePlayer == null && mc.theWorld == null) return;
                final PlayerInstance playermanager$playerinstance1 = this.playerInstanceList.get(k);
            ...
            for (int l = 0; l < this.playerInstancesToUpdate.size(); ++l) {
                if (mc.thePlayer == null && mc.theWorld == null) return;
                final PlayerInstance playermanager$playerinstance2 = this.playerInstancesToUpdate.get(l);
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    for (int i2 = j - i1; i2 <= j + i1; ++i2) {
                        ...
                        if (!this.overlaps(l1 - j1, i2 - k1, i, j, i1)) {
                            ...
                            final PlayerInstance playermanager$playerinstance = this.getPlayerInstance(l1 - j1, i2 - k1, false);
                    ...
                    for (int j1 = j - radius; j1 <= j + radius; ++j1) {
                        for (int k1 = k - radius; k1 <= k + radius; ++k1) {
                            if (Config.isLazyChunkLoading()) {
                                ...
                                final PlayerInstance playermanager$playerinstance1 = this.getPlayerInstance(j1, k1, true);
                    ...
                    for (int l = j - this.playerViewRadius; l <= j + this.playerViewRadius; ++l) {
                        for (int i1 = k - this.playerViewRadius; i1 <= k + this.playerViewRadius; ++i1) {
                            if (!this.overlaps(l, i1, j, k, radius)) {
                                ...
                                final PlayerInstance playermanager$playerinstance = this.getPlayerInstance(l, i1, true);
